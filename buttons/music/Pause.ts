...
export const execute = async (interaction: ButtonInteraction) => {
    ...
    if (queue.playing) await distube.pause(interaction.guildId!); else await distube.resume(interaction.guildId!);

    const updatedComponents = ButtonsArchive.player(!queue.playing);
