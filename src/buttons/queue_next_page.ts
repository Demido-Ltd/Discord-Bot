...
/**
 * This function updates the queue message and shows the next page of songs in the queue.
 * @param {CustomClient} client The bot client
 * @param {ButtonInteraction} interaction The Button Interaction
 * @author Stefan Cucoranu <elpideus@gmail.com>
 * @version 1.0
 */
export default async function queue_next_page (client: CustomClient, interaction: ButtonInteraction) {
    await interaction.deferUpdate();

    const messageEmbedAndComponents = EmbedMessagesArchive.queue(client.distubeAddon.getQueue(interaction.guild!.id)!, MusicQueueManager.currentlySelectedPage(client, interaction.guild!.id, false) as number + 1);
